...
def handle_options():
  ...
  # ═══════════════════════════════════════════════════════════
  #  SECURITY LAYER — Anti-DDoS + IP Whitelist + Access Log
  # ═══════════════════════════════════════════════════════════

  import collections as _col

  # Rate limiting store
  _RATE_STORE = _col.defaultdict(lambda: _col.deque())
  _BLOCKED_IPS = {}       # ip -> unblock_timestamp
  _RATE_LIMIT_REQS = 120  # max requests per window
  _RATE_LIMIT_WINDOW = 60 # seconds
  _BLOCK_DURATION = 300   # 5 minutes block

  # Routes that bypass IP whitelist (everyone can access)
  _PUBLIC_PREFIXES = [
      '/nhan-key-free',
      '/api/gen_free_task',
      '/api/verify',
      '/api/check-device',
      '/api/check_device_approval',
      '/api/check_key',
      '/api/check_free_key_status',
      '/api/submit_device_request',
      '/dang-ky-thiet-bi',
      '/check-ip-key',
      '/api/get_key_ip_info',
      '/healthz',
      '/nhac.mp3', '/nhac2.mp3', '/nhac3.mp3',
      '/logout',
      '/api/add_device_id',
  ]
  ...
  def _log_access(ip, path, method, blocked, reason=''):
      """Ghi nhật ký truy cập vào DB (giữ 800 entries mới nhất)."""
      try:
          db = load_db()
          log = db.get('___ACCESS_LOG___', [])
  ...
  def _get_owner_ips():
      """Trả về tập hợp IP được phép. Bao gồm OWNER_IP từ env và DB whitelist."""
      ips = set()
      env_ip = os.environ.get('OWNER_IP', '').strip()
      if env_ip:
          for raw in env_ip.split(','):
              raw = raw.strip()
              if raw:
                  ips.add(raw)
      try:
          db = load_db()
          wl = db.get('___WHITELIST___', {})
          for ip in wl.keys():
              ips.add(ip)
      except Exception:
          pass
  ...
  def security_layer():
      ...
      # Skip OPTIONS (handled by handle_options)
      if method == 'OPTIONS':
          return None

      # ── Anti-DDoS: check if blocked ──────────────────────────────
      if ip in _BLOCKED_IPS:
          if now < _BLOCKED_IPS[ip]:
              # Log chỉ mỗi 30s để không spam DB
              _log_access(ip, path, method, True, 'DDOS_BLOCKED')
              return _blocked_page(ip)
          else:
              del _BLOCKED_IPS[ip]
      ...
      while dq and now - dq[0] > _RATE_LIMIT_WINDOW:
          dq.popleft()
      dq.append(now)
      if len(dq) > _RATE_LIMIT_REQS:
          _BLOCKED_IPS[ip] = now + _BLOCK_DURATION
          _log_access(ip, path, method, True, 'DDOS_RATE_LIMIT')
          return _blocked_page(ip)

      # ── IP Whitelist: chỉ áp dụng cho route admin ────────────────
      if not _is_public_route(path):
          owner_ips = _get_owner_ips()
          if owner_ips and ip not in owner_ips:
              _log_access(ip, path, method, True, 'IP_NOT_WHITELISTED')
              return _access_denied_page(ip)

      return None


  def _blocked_page(ip):
      html = f"""<!DOCTYPE html><html lang='vi'><head><meta charset='UTF-8'>
  <title>429 - Anti-DDoS</title>
  <style>*{{box-sizing:border-box;margin:0;padding:0;}}
  body{{background:#060b14;color:#e2e8f0;font-family:Inter,sans-serif;
  display:flex;justify-content:center;align-items:center;min-height:100vh;}}
  .box{{text-align:center;padding:48px 36px;background:rgba(10,16,28,0.95);
  border:1px solid rgba(244,63,94,0.3);border-radius:24px;max-width:500px;width:92%;}}
  h1{{color:#f43f5e;font-size:2.2rem;margin-bottom:12px;}}
  p{{color:#94a3b8;line-height:1.7;margin:8px 0;}}
  .ip{{color:#00c8ff;font-family:monospace;font-size:0.9rem;margin-top:16px;}}
  </style></head><body>
  <div class='box'>
  <h1>🛡️ 429</h1>
  <p><b style='color:#f43f5e;'>Anti-DDoS Protection Active</b></p>
  <p>Quá nhiều request từ IP của bạn.<br>IP bị khoá tạm thời 5 phút.</p>
  ...
  *{{box-sizing:border-box;margin:0;padding:0;}}
  body{{background:#060b14;color:#e2e8f0;font-family:'Inter',sans-serif;
  display:flex;justify-content:center;align-items:center;min-height:100vh;
  background-image:radial-gradient(ellipse at 50% 0%,rgba(244,63,94,0.08) 0%,transparent 60%);}}
  .box{{text-align:center;padding:48px 36px;background:rgba(10,16,28,0.92);
  border:1px solid rgba(244,63,94,0.3);border-radius:24px;max-width:500px;width:92%;
  box-shadow:0 30px 80px rgba(0,0,0,0.8),0 0 50px rgba(244,63,94,0.1);}}
  .ico{{font-size:4rem;margin-bottom:20px;}}
  h1{{font-family:'Orbitron',sans-serif;color:#f43f5e;font-size:2rem;
  letter-spacing:3px;margin-bottom:12px;}}
  .sub{{color:#94a3b8;font-size:0.88rem;line-height:1.7;margin-bottom:20px;}}
  .ip-box{{background:rgba(244,63,94,0.06);border:1px solid rgba(244,63,94,0.2);
  border-radius:12px;padding:12px 20px;margin:18px 0;}}
  .ip-label{{font-size:0.7rem;color:#6b7a99;text-transform:uppercase;
  letter-spacing:0.5px;margin-bottom:4px;}}
  .ip-val{{color:#00c8ff;font-family:'Orbitron',sans-serif;font-size:0.85rem;font-weight:700;}}
  .note{{font-size:0.78rem;color:#4a5568;margin-top:20px;line-height:1.7;}}
  .free-link{{color:#00c8ff;font-weight:700;text-decoration:none;
  border:1px solid rgba(0,200,255,0.3);padding:4px 12px;border-radius:7px;}}
  </style></head><body>
  <div class='box'>
  <div class='ico'>🚫</div>
  <h1>403 BLOCKED</h1>
  <div class='sub'>
    Truy cập bị từ chối.<br>
    Địa chỉ IP của bạn <b style='color:#f43f5e;'>không có quyền</b> truy cập trang admin này.
  </div>
  <div class='ip-box'>
  ...
  <div class='note'>
    Nếu bạn cần nhận key free, vui lòng vào:<br>
    <a href='/nhan-key-free' class='free-link'>/nhan-key-free</a>
  </div>
  </div></body></html>"""
      from flask import make_response
...
_ka_thread.start()
  ...
  # --- KEEP-ALIVE STRATEGY 2 & 3: Backup threads để đảm bảo server luôn thức ---
  def _keep_alive_worker2():
      ...
      while True:
          ...
          try:
              ...
              if host2:
                  _ureq.urlopen(host2.rstrip('/') + '/healthz', timeout=12)
          except Exception:
              pass

  def _keep_alive_rotate():
      import time as _tr
      _endpoints = ['/healthz', '/nhan-key-free', '/healthz']
      _idx = 0
      _tr.sleep(240)
      while True:
          _tr.sleep(11 * 60)
          try:
              hostr = os.environ.get('RENDER_EXTERNAL_URL', '')
              if hostr:
                  _ureq.urlopen(hostr.rstrip('/') + _endpoints[_idx % len(_endpoints)], timeout=12)
                  _idx += 1
          except Exception:
              pass

  threading.Thread(target=_keep_alive_worker2, daemon=True).start()
  threading.Thread(target=_keep_alive_rotate, daemon=True).start()
...
def save_db(data):
      # Atomic write: temp file + rename — tránh mất data khi server crash giữa chừng
      ...
      try:
          ...
          try:
              ...
              pass
      # Auto-backup để không mất data khi Render ngủ đông
      try:
          backup_file = os.path.join(DATA_DIR, 'database_keys.backup.json')
...
def get_time_left_str(expiry_timestamp):
    if expiry_timestamp == -1: return "Vĩnh viễn"
    now = time.time()
    ...
    if request.method == 'POST':
        ...
        if k in db and not k.startswith("___"):
            ...
            if info['status'] == 'Đã kích hoạt':
                ...
                all_exp = len(info['used_devices']) > 0 and all(now > exp for exp in info['used_devices'].values() if exp != -1)
...
def list_keys():
    ...
    for k, v in db.items():
        ...
        if v['status'] == "Đã kích hoạt":
            ...
            all_exp = len(v['used_devices']) > 0 and all(now > exp for exp in v['used_devices'].values() if exp != -1)
            if is_full and all_exp:
                v['status'] = "Hết hạn"
                save_db(db)
...
def check_free_key_status():
    ...
    if k in db:
        ...
        now = time.time()
        if info['status'] == 'Đã kích hoạt':
            all_expired = len(info['used_devices']) > 0 and all(now > exp for exp in info['used_devices'].values() if exp != -1)
            if all_expired:
...
# ============================================================
  ...
  @app.route('/api/whitelist_add', methods=['POST'])
  def whitelist_add():
      if not session.get('is_admin'): return jsonify({"status": "error"}), 401
      ip = request.form.get('ip', '').strip()
      label = request.form.get('label', '').strip() or 'Admin'
      if not ip: return jsonify({"status": "error", "msg": "Thiếu địa chỉ IP!"})
      db = load_db()
      wl = db.get('___WHITELIST___', {})
      wl[ip] = {"label": label, "added_at": time.time()}
      db['___WHITELIST___'] = wl
      save_db(db)
      return jsonify({"status": "success"})

  @app.route('/api/whitelist_remove', methods=['POST'])
  def whitelist_remove():
      if not session.get('is_admin'): return jsonify({"status": "error"}), 401
      ip = request.form.get('ip', '').strip()
      db = load_db()
      wl = db.get('___WHITELIST___', {})
      if ip in wl: del wl[ip]
      db['___WHITELIST___'] = wl
      save_db(db)
      return jsonify({"status": "success"})

  @app.route('/api/whitelist_list', methods=['GET'])
  def whitelist_list():
      if not session.get('is_admin'): return jsonify([]), 401
      db = load_db()
      wl = db.get('___WHITELIST___', {})
      env_ip = os.environ.get('OWNER_IP', '').strip()
      result = []
      for ip, info in wl.items():
          result.append({"ip": ip, "label": info.get("label", "—"),
              "added_at": format_ts(info.get("added_at", 0)), "env": False})
      if env_ip:
          for raw in env_ip.split(','):
              raw = raw.strip()
              if raw and raw not in wl:
                  result.append({"ip": raw, "label": "ENV: OWNER_IP", "added_at": "—", "env": True})
      return jsonify(result)

  @app.route('/api/whitelist_status', methods=['GET'])
  def whitelist_status():
      if not session.get('is_admin'): return jsonify({"enabled": False}), 401
      owner_ips = _get_owner_ips()
      return jsonify({"enabled": bool(owner_ips), "count": len(owner_ips)})

  @app.route('/api/my_current_ip', methods=['GET'])
  def my_current_ip():
      if not session.get('is_admin'): return jsonify({"status": "error"}), 401
  ...
  def get_access_log():
      ...
      db = load_db()
      log = db.get('___ACCESS_LOG___', [])
      return jsonify(list(reversed(log[-500:])))

  @app.route('/api/access_log_clear', methods=['POST'])
  ...
  def block_ip_manual():
      ...
      if ip: _BLOCKED_IPS[ip] = time.time() + (minutes * 60)
      return jsonify({"status": "success"})

  @app.route('/api/unblock_ip', methods=['POST'])
  def unblock_ip_manual():
      if not session.get('is_admin'): return jsonify({"status": "error"}), 401
      ip = request.form.get('ip', '').strip()
      if ip in _BLOCKED_IPS: del _BLOCKED_IPS[ip]
      return jsonify({"status": "success"})
  ...
  def list_blocked_ips():
      ...
      for ip, until in list(_BLOCKED_IPS.items()):
          if now < until:
              result.append({"ip": ip, "until": format_ts(until), "remaining_s": int(until - now)})
          else:
              del _BLOCKED_IPS[ip]
      return jsonify(result)
...
<div class="nav-dropdown" id="navDrop">
  ...
  <button class="nav-item" onclick="sw('settings')"><i class="fa-solid fa-gear"></i> Settings</button>
    <button class="nav-item" onclick="sw('whitelist')"><i class="fa-solid fa-shield-check"></i> IP Whitelist</button>
    <button class="nav-item" onclick="sw('accesslog')"><i class="fa-solid fa-scroll"></i> Nhật Ký Web</button>
...
// ---- WHITELIST MANAGEMENT ----
  function loadWhitelist() {
    Promise.all([
      fetch('/api/whitelist_list').then(r=>r.json()),
      fetch('/api/whitelist_status').then(r=>r.json())
    ]).then(([items, status]) => {
      const statusBox = document.getElementById('wlStatusBox');
      if(statusBox) {
        if(status.enabled) {
          statusBox.innerHTML = `<div style="background:rgba(16,217,138,0.08);border:1px solid rgba(16,217,138,0.25);border-radius:10px;padding:10px 14px;font-size:0.82rem;color:var(--success);font-weight:700;"><i class="fa-solid fa-shield-check"></i> Whitelist ĐANG BẬT — ${status.count} IP được phép</div>`;
        } else {
          statusBox.innerHTML = `<div style="background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.25);border-radius:10px;padding:10px 14px;font-size:0.82rem;color:var(--warn);font-weight:700;"><i class="fa-solid fa-triangle-exclamation"></i> Whitelist TẮT — Thêm ít nhất 1 IP để bật bảo vệ</div>`;
        }
      }
      const list = document.getElementById('wlList');
      if(!list) return;
      if(!items.length) {
        list.innerHTML = '<div style="color:var(--muted);font-size:0.82rem;text-align:center;padding:16px 0;"><i class="fa-solid fa-shield-slash"></i> Chưa có IP nào trong whitelist</div>';
        return;
      }
      let html = '<div class="tbl-wrap"><table><thead><tr><th>Địa chỉ IP</th><th>Nhãn</th><th>Thêm lúc</th><th></th></tr></thead><tbody>';
      items.forEach(it => {
        const envBadge = it.env ? '<span style="background:rgba(245,158,11,0.15);color:var(--warn);padding:2px 7px;border-radius:5px;font-size:0.68rem;font-weight:800;margin-left:6px;">ENV</span>' : '';
        html += `<tr>
          <td style="font-family:'Orbitron',sans-serif;font-size:0.74rem;color:var(--success);">${it.ip}${envBadge}</td>
          <td style="color:var(--muted);font-size:0.8rem;">${it.label}</td>
          <td style="color:var(--muted);font-size:0.74rem;">${it.added_at}</td>
          <td>${it.env ? '<span style="color:var(--muted);font-size:0.72rem;">Từ ENV</span>' : `<button class="btn-sm btn-sm-red" onclick="removeFromWhitelist('${it.ip}')"><i class="fa-solid fa-trash"></i></button>`}</td>
        </tr>`;
      });
      ...
      const list = document.getElementById('wlList');
      if(list) list.innerHTML = '<div style="color:var(--danger);">Lỗi tải whitelist.</div>';
    });
  }

  function getMyCurrentIP() {
    fetch('/api/my_current_ip').then(r=>r.json()).then(d=>{
      if(d.ip) {
        const inp = document.getElementById('wlIPInput');
        if(inp) inp.value = d.ip;
        const lbl = document.getElementById('wlLabelInput');
        if(lbl && !lbl.value) lbl.value = 'Owner';
      }
    }).catch(()=>{});
  }

  function addToWhitelist() {
    const ip = (document.getElementById('wlIPInput')||{}).value ? document.getElementById('wlIPInput').value.trim() : '';
    const label = (document.getElementById('wlLabelInput')||{}).value ? document.getElementById('wlLabelInput').value.trim() : 'Admin';
    const alertEl = document.getElementById('wlAlert');
    if(!ip) {
      if(alertEl) { alertEl.style.cssText='display:block;background:rgba(244,63,94,0.1);border:1px solid rgba(244,63,94,0.3);color:var(--danger);border-radius:10px;padding:10px 13px;font-size:0.82rem;font-weight:700;margin-bottom:12px;'; alertEl.innerHTML='<i class="fa-solid fa-triangle-exclamation"></i> Vui lòng nhập IP!'; }
      return;
    }
    fetch('/api/whitelist_add',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'ip='+encodeURIComponent(ip)+'&label='+encodeURIComponent(label)})
    .then(r=>r.json()).then(d=>{
      if(d.status==='success') {
        if(alertEl){ alertEl.style.cssText='display:block;background:rgba(16,217,138,0.08);border:1px solid rgba(16,217,138,0.25);color:var(--success);border-radius:10px;padding:10px 13px;font-size:0.82rem;font-weight:700;margin-bottom:12px;'; alertEl.innerHTML='<i class="fa-solid fa-check-circle"></i> Đã thêm IP '+ip+' vào whitelist!'; }
        if(document.getElementById('wlIPInput')) document.getElementById('wlIPInput').value='';
        if(document.getElementById('wlLabelInput')) document.getElementById('wlLabelInput').value='';
        loadWhitelist();
  ...
  function removeFromWhitelist(ip) {
    if(!confirm('Xóa IP '+ip+' khỏi whitelist?')) return;
    fetch('/api/whitelist_remove',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'ip='+encodeURIComponent(ip)})
    .then(r=>r.json()).then(()=>loadWhitelist());
  }

  function loadBlockedIPs() {
    fetch('/api/blocked_ips').then(r=>r.json()).then(items=>{
      const el = document.getElementById('blockedIPList');
      if(!el) return;
      if(!items.length) {
        el.innerHTML = '<div style="color:var(--muted);font-size:0.82rem;text-align:center;padding:16px 0;">Không có IP nào đang bị block.</div>';
        return;
      }
      let html = '<div class="tbl-wrap"><table><thead><tr><th>IP</th><th>Hết hạn</th><th>Còn lại</th><th></th></tr></thead><tbody>';
      items.forEach(it=>{
        html += '<tr><td style="font-family:monospace;color:var(--danger);font-size:0.8rem;">'+it.ip+'</td><td style="color:var(--muted);font-size:0.75rem;">'+it.until+'</td><td style="color:var(--warn);font-size:0.8rem;">'+it.remaining_s+'s</td><td><button class="btn-sm btn-sm-green" onclick="unblockIP(''+it.ip+'')"><i class="fa-solid fa-lock-open"></i> Bỏ block</button></td></tr>';
      });
      html += '</tbody></table></div>';
      el.innerHTML = html;
    });
  ...
  function unblockIP(ip) {
    fetch('/api/unblock_ip',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'ip='+encodeURIComponent(ip)}).then(()=>loadBlockedIPs());
  }
  ...
  function loadAccessLog() {
    ...
    if(!el) return;
    el.innerHTML = '<div style="color:var(--muted);font-size:0.82rem;text-align:center;padding:20px 0;"><div class="spinner" style="margin:0 auto 10px;width:22px;height:22px;border-width:2px;"></div>Đang tải nhật ký...</div>';
    fetch('/api/access_log').then(r=>r.json()).then(data=>{
      _logData = data;
      renderLog();
    }).catch(()=>{ el.innerHTML='<div style="color:var(--danger);">Lỗi tải nhật ký.</div>'; });
  }

  function filterLog(mode) { _logFilter = mode; renderLog(); }
  ...
  function renderLog() {
    ...
    if(!el) return;
    var data = _logData;
    var totalBlocked = data.filter(function(x){return x.blocked;}).length;
    if(sumEl) {
      sumEl.innerHTML = '<div style="display:flex;gap:10px;flex-wrap:wrap;font-size:0.8rem;">'
        +'<span style="background:rgba(244,63,94,0.1);color:var(--danger);border:1px solid rgba(244,63,94,0.25);padding:5px 12px;border-radius:8px;font-weight:700;"><i class="fa-solid fa-ban"></i> Bị chặn: '+totalBlocked+'</span>'
        +'<span style="background:rgba(16,217,138,0.08);color:var(--success);border:1px solid rgba(16,217,138,0.2);padding:5px 12px;border-radius:8px;font-weight:700;"><i class="fa-solid fa-check"></i> Cho phép: '+(data.length-totalBlocked)+'</span>'
        +'<span style="background:rgba(0,200,255,0.06);color:var(--blue);border:1px solid rgba(0,200,255,0.2);padding:5px 12px;border-radius:8px;font-weight:700;"><i class="fa-solid fa-list"></i> Tổng: '+data.length+'</span>'
        +'</div>';
    }
    if(_logFilter === 'blocked') data = data.filter(function(x){return x.blocked;});
    if(!data.length) {
      el.innerHTML = '<div style="color:var(--muted);font-size:0.82rem;text-align:center;padding:20px 0;"><i class="fa-solid fa-check-circle" style="color:var(--success);"></i> Không có log nào.</div>';
      return;
    }
    var html = '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;font-size:0.75rem;">'
      +'<thead><tr style="background:rgba(4,8,16,0.9);">'
      +'<th style="padding:9px 10px;color:var(--blue);text-align:left;white-space:nowrap;">Thời Gian</th>'
      +'<th style="padding:9px 10px;color:var(--blue);text-align:left;">IP</th>'
      +'<th style="padding:9px 10px;color:var(--blue);text-align:left;">Đường Dẫn</th>'
      +'<th style="padding:9px 10px;color:var(--blue);text-align:left;">Trạng Thái</th>'
      +'<th style="padding:9px 10px;color:var(--blue);text-align:left;">Lý Do</th>'
      +'</tr></thead><tbody>';
    var reasonColors = {'DDOS_BLOCKED':'#f43f5e','DDOS_RATE_LIMIT':'#f43f5e','IP_NOT_WHITELISTED':'#f59e0b'};
    data.forEach(function(entry){
      var badge = entry.blocked
        ? '<span style="background:rgba(244,63,94,0.1);color:var(--danger);border:1px solid rgba(244,63,94,0.25);padding:2px 7px;border-radius:5px;font-weight:800;"><i class=\"fa-solid fa-ban\"></i> BỊ CHẶN</span>'
        : '<span style="background:rgba(16,217,138,0.08);color:var(--success);border:1px solid rgba(16,217,138,0.2);padding:2px 7px;border-radius:5px;font-weight:800;"><i class=\"fa-solid fa-check\"></i> OK</span>';
      var rc = reasonColors[entry.reason] || 'var(--muted)';
      var pathStr = (entry.path||'').substring(0,35);
      html += '<tr style="border-top:1px solid rgba(255,255,255,0.03);">'
        +'<td style="padding:8px 10px;color:var(--muted);white-space:nowrap;">'+(entry.t||'—')+'</td>'
        +'<td style="padding:8px 10px;font-family:monospace;font-size:0.78rem;color:var(--blue);">'+(entry.ip||'—')+'</td>'
        +'<td style="padding:8px 10px;color:#94a3b8;max-width:140px;word-break:break-all;">'+pathStr+'</td>'
        +'<td style="padding:8px 10px;">'+badge+'</td>'
        +'<td style="padding:8px 10px;color:'+rc+';font-size:0.7rem;">'+(entry.reason||'—')+'</td>'
        +'</tr>';
    });
    html += '</tbody></table></div>';
    el.innerHTML = html;
  ...
  function clearAccessLog() {
    if(!confirm('Xóa toàn bộ nhật ký truy cập?')) return;
    fetch('/api/access_log_clear',{method:'POST'}).then(function(){
      _logData = []; renderLog();
    });
  }
...
"""
  ...
  WHITELIST_TAB_HTML = """
      <!-- TAB WHITELIST -->
      <div id="tab-whitelist" class="tab">
        <div class="card">
          <div class="card-title"><i class="fa-solid fa-shield-check" style="color:#10d98a;"></i> Quản Lý IP Whitelist</div>
          <div style="background:rgba(16,217,138,0.05);border:1px solid rgba(16,217,138,0.2);border-radius:12px;padding:13px 15px;margin-bottom:14px;font-size:0.8rem;color:#94a3b8;line-height:1.75;">
            <b style="color:#10d98a;"><i class="fa-solid fa-circle-info"></i> Cách hoạt động:</b><br>
            Khi có ít nhất 1 IP trong danh sách, chỉ IP đó mới vào được trang admin (<code style="color:#00c8ff;">/</code>).<br>
            Trang key free (<code style="color:#00c8ff;">/nhan-key-free</code>) luôn mở cho tất cả.<br>
            Thiết lập OWNER_IP qua biến môi trường trên Render để bật ngay khi deploy.
          </div>
          <div id="wlStatusBox" style="margin-bottom:12px;"></div>
          <div style="display:flex;gap:9px;margin-bottom:12px;">
            <button class="btn-sm btn-sm-blue" onclick="getMyCurrentIP()" style="padding:9px 14px;"><i class="fa-solid fa-location-dot"></i> Lấy IP hiện tại của tôi</button>
          </div>
          <div style="display:flex;gap:9px;margin-bottom:14px;">
            <input type="text" id="wlIPInput" placeholder="Nhập địa chỉ IP cần thêm..." style="flex:1;padding:11px 13px;background:rgba(4,8,16,0.85);border:1px solid rgba(255,255,255,0.08);border-radius:10px;color:var(--text);font-size:0.85rem;outline:none;font-family:'Inter',sans-serif;">
            <input type="text" id="wlLabelInput" placeholder="Nhãn (VD: Nhà)" style="width:130px;padding:11px 13px;background:rgba(4,8,16,0.85);border:1px solid rgba(255,255,255,0.08);border-radius:10px;color:var(--text);font-size:0.85rem;outline:none;font-family:'Inter',sans-serif;">
            <button class="btn-sm btn-sm-green" onclick="addToWhitelist()" style="padding:9px 15px;white-space:nowrap;"><i class="fa-solid fa-plus"></i> Thêm</button>
          </div>
          <div id="wlAlert" style="display:none;margin-bottom:10px;"></div>
          <div id="wlList">
            <div style="color:var(--muted);font-size:0.82rem;text-align:center;padding:20px 0;">
              <div class="spinner" style="margin:0 auto 10px;width:22px;height:22px;border-width:2px;"></div>Đang tải...
            </div>
          </div>
        </div>
        <div class="card" style="margin-top:14px;">
          <div class="card-title"><i class="fa-solid fa-ban" style="color:#f43f5e;"></i> IP Đang Bị Block (Anti-DDoS)</div>
          <div style="display:flex;gap:9px;margin-bottom:12px;">
            <button class="btn-sm btn-sm-blue" onclick="loadBlockedIPs()" style="padding:9px 14px;"><i class="fa-solid fa-rotate"></i> Làm mới</button>
          </div>
          <div id="blockedIPList">
            <div style="color:var(--muted);font-size:0.82rem;text-align:center;padding:16px 0;">Không có IP nào đang bị block.</div>
          </div>
        </div>
      </div>
  """
  ...
  ACCESSLOG_TAB_HTML = """
      <!-- TAB ACCESS LOG -->
      <div id="tab-accesslog" class="tab">
        <div class="card">
          <div class="card-title"><i class="fa-solid fa-scroll" style="color:#f59e0b;"></i> Nhật Ký Web — Lịch Sử Xâm Nhập</div>
          <div style="background:rgba(245,158,11,0.05);border:1px solid rgba(245,158,11,0.2);border-radius:12px;padding:11px 14px;margin-bottom:14px;font-size:0.79rem;color:#94a3b8;line-height:1.7;">
            <b style="color:#f59e0b;"><i class="fa-solid fa-shield-halved"></i> Ghi lại:</b> Mọi IP cố truy cập trái phép, vượt rate limit, hoặc bị từ chối đều được ghi lại tại đây.
          </div>
          <div style="display:flex;gap:9px;margin-bottom:14px;flex-wrap:wrap;">
            <button class="btn-sm btn-sm-blue" onclick="loadAccessLog()" style="padding:9px 14px;"><i class="fa-solid fa-rotate"></i> Làm mới</button>
            <button class="btn-sm btn-sm-warn" onclick="filterLog('blocked')" style="padding:9px 14px;"><i class="fa-solid fa-ban"></i> Chỉ bị chặn</button>
            <button class="btn-sm btn-sm-blue" onclick="filterLog('all')" style="padding:9px 14px;"><i class="fa-solid fa-list"></i> Tất cả</button>
            <button class="btn-sm btn-sm-red" onclick="clearAccessLog()" style="padding:9px 14px;"><i class="fa-solid fa-trash"></i> Xóa log</button>
          </div>
          <div id="logSummary" style="margin-bottom:12px;"></div>
          <div id="accessLogTable">
            <div style="color:var(--muted);font-size:0.82rem;text-align:center;padding:20px 0;">
              <div class="spinner" style="margin:0 auto 10px;width:22px;height:22px;border-width:2px;"></div>Đang tải nhật ký...
            </div>
          </div>
        </div>
      </div>
  """
  UI_TEMPLATE = HTML_P1 + HTML_P2 + HTML_P3 + WHITELIST_TAB_HTML + ACCESSLOG_TAB_HTML + HTML_P5
